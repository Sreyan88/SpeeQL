...
def preprocess_data_rec_engine(status):
    ...
    rows = group_cleaned_purchased.CustomerID.astype('category', CategoricalDtype(categories = customers)).cat.codes
    cols = group_cleaned_purchased.StockCode.astype('category', CategoricalDtype(categories = products)).cat.codes
